"""
    xkp: 2023/6/6 This script is added only to try unequally distributed bootstrap.
    as 200 = [100, 50, 50] when k = 3, beta = 0
"""
...
def main(args):
    ...
    for i in range(args.k):
        ...
        # train
        if args.unequal:
            print("Unequal training")
            assert len(args.dis) == args.k
            each_epoch = args.dis[i]

        print(f"Start training for {each_epoch} epochs")
    ...
    misc.save_model(
                args=args, model=None, model_without_ddp=model, optimizer=optimizer,    # single GPU
                loss_scaler=loss_scaler, epoch='bmae_pretrain_unequal')
